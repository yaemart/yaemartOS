...
> **代码卫生周**（W51 尾或 W52 前后排）：+0.45 人日（~~F-8 footgun~~（W49 D2 紧急 hotfix）+ F-10–F-17 P3 + 测试缺口 T-4），明细见 [`docs/code-hygiene-backlog.md`](./code-hygiene-backlog.md)
...
| ~~**F-8 GitHub Actions `set +e` 后未恢复**~~（W49 D1 `/review` P2 → **W49 D2 紧急升级 P0 + ✅ 已修**） | 原因：cron 第 1 次 manual trigger 25s ✅ success 暴露**真实影响是 P0** —— smoke step 末尾 `echo "exit_code=$?"` 让 `steps.smoke.outcome` 永远 `success`（echo 自身 exit 0 覆盖），workflow 永远绿，silent fail 不通知不阻塞，cron 解锁判定彻底失效                                                                                                                                                                                                                       | W49 D2 hotfix：smoke step 改 `ec=$?; echo exit_code=$ec; exit $ec` 模式（直接 commit 到 main，commit `e3cb609`），同步从卫生周方案 A 移出，下次 manual trigger 验证 outcome 正确反映真实 exit code                                                           |
| ~~**F-19 smoke 早退路径不写 stdout JSON**~~（W49 D2 hotfix 期间新发现，**当晚 ✅ 已修**，F-8 孪生）    | 原因：F-8 hotfix 后第 2 次 manual trigger 文 ✅ workflow ❌，但 artifact `smoke-output.json` 是 0 字节空文件 —— smoke 脚本 `envOrDie()` first-fail 直接 `process.exit(2)`，stdout 一字未写。Slack notify 走 jq fallback banner、artifact 无 hardFailures，运维诊断断链。**Plus**：发现 `scripts/staging/smoke-chat-tool.spec.ts` 因 vitest include glob 是相对 `apps/api/` cwd 的 `scripts/**`，**W49 D1 写的 drift spec 从未在 CI 跑过**（dead test bug）               | W49 D2 hotfix：（1）`envOrDie` → `requireEnv(key, missing)` collect-all + missing>0 路径调 `finalize(..., 2)` 复用 stdout JSON 输出；（2）`vitest.config.ts` include 加 `'../../scripts/**/*.spec.ts'` 救活 dead test；（3）新增 4 条 drift 断言防回归       |
