...
| ~~**F-8 GitHub Actions `set +e` 后未恢复**~~（W49 D1 `/review` P2 → **W49 D2 紧急升级 P0 + ✅ 已修**） | 原因：cron 第 1 次 manual trigger 25s ✅ success 暴露**真实影响是 P0** —— smoke step 末尾 `echo "exit_code=$?"` 让 `steps.smoke.outcome` 永远 `success`（echo 自身 exit 0 覆盖），workflow 永远绿，silent fail 不通知不阻塞，cron 解锁判定彻底失效                                                                                                                                                                                                                       | W49 D2 hotfix：smoke step 改 `ec=$?; echo exit_code=$ec; exit $ec` 模式（直接 commit 到 main，commit `<hash>`），同步从卫生周方案 A 移出，下次 manual trigger 验证 outcome 正确反映真实 exit code                                                            |
