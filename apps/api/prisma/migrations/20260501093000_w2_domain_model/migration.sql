...
-- Wrapped in DO block with EXCEPTION WHEN OTHERS so any failure (type not visible,
-- cast error, etc.) is silently absorbed and never aborts the migration transaction.
DO $$
BEGIN
